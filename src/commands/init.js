export function getInitScript() {
  ...
  const lines = [
    ...
    '    local _session_id="$1"',
    '    shift',
    '    # Note: --profile must be passed as the first remaining arg if needed.',
    '    # It is forwarded to `command cloak resume` only; "$@" goes to claude.',
    '    local _profile_args=""',
    '    if [ "$1" = "--profile" ] || [ "$1" = "-p" ]; then',
    '      _profile_args="--profile $2"',
    '      shift 2',
    '    fi',
    '    local output',
    '    output=$(command cloak resume --print-env ${_profile_args} "$_session_id")',
    '    local exit_code=$?',
    '    if [ $exit_code -eq 0 ]; then',
    '      eval "$output"',
    '      command cloak context-bar resume',
    '      command claude --resume "$_session_id" "$@"',
    '    fi',
    '  else',
    '    command cloak "$@"',
    '  fi',
